...
function recreatePictureWindow(windowId, windowData, fRole) {
  ...
  if (isDoor) {
    console.debug(`[STORAGE] Recreating door window: ${windowId}`);
    
    // Get door state from doorKeySystem
    const doorState = getDoorState(windowId);
    
    // Extract door-specific settings
    const doorKeySettings = specialConfig?.doorKeySettings || {};
    const isEncrypted = doorKeySettings.encrypted || false;
    
    // Determine door state from saved data
    let state = 'closed';
    let isLocked = true;
    ...
    if (!closedImagePath && specialConfig?.pictureSettings?.imagePath) {
      closedImagePath = specialConfig.pictureSettings.imagePath;
    }
    if (!closedImagePath && contentConfig?.path) {
      closedImagePath = contentConfig.path;
    }
    
    // Create door with proper state
    const doorOptions = {
      closedImagePath,
      openedImagePath,
      initialState: state,
    ...
    console.log(`[STORAGE] Creating door ${windowId} with state:`, doorOptions);
  ...
  // Handle regular picture windows
  let imagePath = null;
  let fitMode = 'fill';

  // Check specialConfig first (for picture windows)
  ...
  if (!imagePath) {
    ...
    console.log(`[STORAGE] No image path found for ${windowId}, using default image: ${imagePath}`);
  }
