...
import { serializeWindow } from '../storage/windowStorage.js';
import { exportRelationshipState } from './doorKeySystem.js';
import { exportGameLogicState } from './gameLogic.js';
import { triggerStateSaved, triggerStateLoaded } from '../core/callbacks/gameStateCallbacks.js';
...
function getSaveFilePath(customPath = null) {
  if (customPath) {
    // Validate custom path using path security validator
    const gameDataRoot = getDefaultGameDataDirectory();
    const resolvedCustomPath = path.isAbsolute(customPath) ? customPath : path.join(process.cwd(), customPath);
    
    const pathValidation = validateAndResolvePath(resolvedCustomPath, gameDataRoot, gameDataRoot);
    if (!pathValidation.isValid) {
      console.error(`[GAME_STATE] Invalid custom save path: ${pathValidation.error}`);
      
      // Log security event
      securityAuditSystem.logSecurityEvent('path_violation', 'high', {
        component: 'GameStateManager',
        function: 'getSaveFilePath',
        violationType: 'invalid_save_path',
        mitigationAction: 'path_rejected',
        inputData: customPath
      }, {
        requestedPath: resolvedCustomPath,
        gameDataRoot
      });
...
function ensureStorageDirectory() {
  ...
  const gameDataRoot = getDefaultGameDataDirectory();
  const pathValidation = validateAndResolvePath(storageDir, gameDataRoot, gameDataRoot);
  if (!pathValidation.isValid) {
    console.error(`[GAME_STATE] Invalid storage directory path: ${pathValidation.error}`);
    
    // Log security event
    securityAuditSystem.logSecurityEvent('path_violation', 'high', {
      component: 'GameStateManager',
      function: 'ensureStorageDirectory',
      violationType: 'invalid_storage_path',
      mitigationAction: 'directory_creation_blocked',
      inputData: storageDir
    }, {
      gameDataDir,
      gameDataRoot
    });
    
    throw new Error(`Invalid storage directory path: ${pathValidation.error}`);
  }

  if (!fs.existsSync(pathValidation.resolvedPath)) {
    try {
      fs.mkdirSync(pathValidation.resolvedPath, { recursive: true });
      console.log(`[GAME_STATE] Created storage directory: ${pathValidation.resolvedPath}`);
      
      // Log successful directory creation
      securityAuditSystem.logSecurityEvent('file_operation', 'low', {
        component: 'GameStateManager',
        function: 'ensureStorageDirectory',
        violationType: 'authorized_directory_creation',
        mitigationAction: 'directory_created',
        inputData: pathValidation.resolvedPath
      });
      
    } catch (error) {
      console.error(`[GAME_STATE] Failed to create storage directory:`, error);
      
      // Log security event for directory creation failure
      securityAuditSystem.logSecurityEvent('file_operation', 'medium', {
        component: 'GameStateManager',
        function: 'ensureStorageDirectory',
        violationType: 'directory_creation_failed',
        mitigationAction: 'error_thrown',
...
export async function saveGameState(savePath = null, options = {}) {
  ...
  // Register memory cleanup for save operation
  const cleanupId = `gameStateSave_${Date.now()}`;
  memoryManager.registerCleanupHandler(cleanupId, () => {
    console.log('[GAME_STATE] Cleaning up save operation resources');
  });
  ...
  try {
    ...
    if (failedWindows.length > 0) {
      ...
      // Log security event for serialization failures
      securityAuditSystem.logSecurityEvent('data_operation', 'medium', {
        component: 'GameStateManager',
        function: 'saveGameState',
        violationType: 'window_serialization_failures',
        mitigationAction: 'partial_save_continued',
        inputData: JSON.stringify(failedWindows)
      }, {
        failedCount: failedWindows.length,
        totalWindows: windows.size
      });
    }
    ...
    securityAuditSystem.logSecurityEvent('data_operation', 'low', {
      component: 'GameStateManager',
      function: 'saveGameState',
      violationType: 'authorized_save_operation',
      mitigationAction: 'state_saved_successfully',
      inputData: JSON.stringify({ windowCount: serializedWindows.length, filePath: saveFilePath }).substring(0, 200)
    }, {
      saveFilePath,
      windowCount: serializedWindows.length
    });
    ...
    // Log security event for save failure
    securityAuditSystem.logSecurityEvent('data_operation', 'high', {
      component: 'GameStateManager',
      function: 'saveGameState',
      violationType: 'save_operation_failed',
    ...
    memoryManager.unregisterCleanupHandler(cleanupId);
  }
}
